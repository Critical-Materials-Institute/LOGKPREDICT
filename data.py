...
# fedzah
def myMolFromSmiles(smiles0):
    mol0 = Chem.MolFromSmiles(smiles0,sanitize=False)
    mol0.UpdatePropertyCache(strict=False)
    Chem.SanitizeMol(mol0,Chem.SanitizeFlags.SANITIZE_FINDRADICALS|Chem.SanitizeFlags.SANITIZE_KEKULIZE|Chem.SanitizeFlags.SANITIZE_SETAROMATICITY|Chem.SanitizeFlags.SANITIZE_SETCONJUGATION|Chem.SanitizeFlags.SANITIZE_SETHYBRIDIZATION|Chem.SanitizeFlags.SANITIZE_SYMMRINGS,catchErrors=True)
    return mol0
...
class MoleculeDatapoint:
    ...
    def mol(self) -> List[Chem.Mol]:
        ...
        mol = [SMILES_TO_MOL.get(s, myMolFromSmiles(s)) for s in self.smiles]
